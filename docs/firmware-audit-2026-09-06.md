...
- 2026-09-07 追補：提供された相談画面 2 枚を確認。出荷個体のファーム、各報告の完全な操作履歴、実機の再現試験は未確認。下記はコード・生成設定から確定できる事実と、利用者の症状の原因候補を分けて記載する。
...
## 2026-09-07：利用者報告を踏まえた追補

提供された相談画面に基づき、個人名・画像・会話全文は転載せず、汎用的な対処をガイドに反映した。

| 観察できた内容 | 判定と対応 |
| --- | --- |
| `The port is already open` | Web Serial の開閉状態エラー。ブラウザ側で接続試行が重なる／失敗後に未解放などが候補。画像だけで特定の Web UI バグと断定はできず、ファーム初期化を第一手にしない |
| Windows コピーの `0x80070022` | 媒体に関するエラー。UF2 完了時の再起動でも起こり得るが、画像だけで書き込み成功は確認できない。通常起動と更新版の確認を条件にする |
| `Corcell_L-xiao_ble_zmk.uf2` という名前 | 現行 main の成果物名と一致する。DYA 版と取り違えた可能性があるが、改名・旧版もあり得るため Build URL で確認する |
| BLE で候補が出ず、左右再起動後に見つかったとの報告 | 広告／接続状態の変化と整合するが因果関係は未確定。毎回リセット必須という仕様にはしない。今回の検出用広告の修正を実機確認する |
| dev preview では認識が進んだとの報告 | Web UI の版や接続状態の差が候補。通常版と dev preview を同時に開かず URL を記録して比較。ファームの USB transport 不足とは区別する |

この追補はドキュメントのみ。ビルド検証済みファームは `e073cfb` のままで、Windows のコピーやブラウザ開閉エラーを修正済みと主張しない。
