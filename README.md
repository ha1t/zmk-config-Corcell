...
### 2026-09-17 — 左手のスリープ復帰

- 左手がスリープに入るとキーを押しても復帰せず、リセットが必要になる問題を修正しました。
  XIAO のボード定義が SPI2 をキー読み取りと同じピンで有効にしており、スリープ直前に
  SPI2 の省電力設定が割り込みピンの起床設定を消していました。右手には以前から
  SPI2 無効化が入っていたため右手だけ復帰できていました。（対象: 左手）
...
初期配列のL1・L2・L3キーは、押している間のレイヤー切替に加えて、それぞれ英数（`LANG2`）・かな（`LANG1`）・変換（`INT_HENKAN`）を送信します。離すとキー信号とレイヤーを解除します。KeyCheckなど通常のキー検査で、Studio接続なしに親指キーの押下・解放を確認できます。

文字キーは送信しませんが、IMEの入力モード切替や変換は実行されます。英数・かなはmacOSの日本語入力向けの名称です。OS・入力ソース・ブラウザによってキー名や検出可否は異なります。短押しでI/Aになるレイヤー5キーは従来どおりです。
